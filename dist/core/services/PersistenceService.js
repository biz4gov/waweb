...
class PersistenceService {
    ...
    async findOrCreateContact(accountId, phoneNumber, name) {
    ...
    async updateAgentAssignmentTimestamp(agentId) {
    ...
    async findOrCreateConversation(accountId, channelId, contactId, agentId) {
    ...
    // CREATE
    async createWebhook(accountId, eventType, targetUrl) {
    ...
    // READ (List)
    async listWebhooksByAccount(accountId) {
        const query = `
    ...
    // READ (By ID)
    async findWebhookById(webhookId, accountId) {
        const query = `
      SELECT id, event_type, target_url, is_active, created_at FROM webhooks
    ...
    // UPDATE
    async updateWebhook(webhookId, accountId, data) {
        ...
        if (findRes.rowCount === 0)
            return null; // Não encontrado ou não pertence à conta
        const fields = Object.entries(data).filter(([_, value]) => value !== undefined);
        if (fields.length === 0)
            return this.findWebhookById(webhookId, accountId);
    ...
    async findAgentByEmail(accountId, email) {
