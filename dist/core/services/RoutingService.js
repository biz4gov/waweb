...
class RoutingService {
     ...
     */
    async getAssignedAgent(accountId, contactId, channelId) {
    ...
    async findTriageBot(accountId) {
        const query = `SELECT id FROM agents WHERE account_id = $1 AND type = 'BOT' LIMIT 1`;
        const res = await pool.query(query, [accountId]);
        if (res.rowCount === 0)
            throw new Error('Nenhum Triage Bot configurado para a conta.');
        return res.rows[0].id;
    }
    async findNextAvailableHumanAgent(accountId) {
